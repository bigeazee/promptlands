 ...
 *
 *   markSolid   you stand NEXT TO a station or a guide, never on top of it
 ...
 *
 * Error messages name the definition's key - a station or gate `id`, a guide's
 ...
 *
 * `kind` is the noun the error messages use - "station", "guide" - because a
 ...
 * zone and an id would be busywork, by `zone`; guides are the latter and pass
...
export function markSolid(grid, items, kind) {
  ...
  if (typeof kind !== "string" || kind === "") {
    throw new Error('markSolid: needs a kind, the noun for these items - "station" or "guide".');
  }
 ...
 *
 * `id` where there is one. Where there is not - a guide, of which there is
